...
class Media(Document):
    # Define fields without using 'required=True' directly.
    file_id = StringField(missing=None)
    file_ref = StringField(missing=None)
    file_name = StringField(missing=None)
    file_size = IntegerField(missing=None)
    file_type = StringField(missing=None)
    mime_type = StringField(missing=None)
    caption = StringField(missing=None)

    class Meta:
        collection_name = COLLECTION_NAME

    # We can use the `missing=None` in place of `required=True` to allow for empty fields
